...
class PlacesViewController: ViewController, UISearchBarDelegate, ArticlePopoverViewControllerDelegate, PlaceSearchSuggestionControllerDelegate, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate, ArticlePlaceViewDelegate, UIGestureRecognizerDelegate, HintPresenting {
    ...
    func region(thatFits location: CLLocation) -> MKCoordinateRegion {
        return region(thatFits: [location.coordinate])
    }
    ...
    @objc public func showLocation(_ location: CLLocation, withTitle title: String?) {
        ...
        if let title {
            ...
            currentSearch = PlaceSearch(filter: .top, type: .location, origin: .user, sortStyle: .none, string: nil, region: region, localizedDescription: title, searchResult: searchResult)
        } else {
            performDefaultSearch(withRegion: region)
        }
    }
